...
function shortenName(name) {
    ...
    //temp_name = name_array[0].replace(/ /g, '\\n');
    /*for (var j = 0; j < name_array.length; j++) {
        temp_name += toTitleCase(name_array[j]);
        (j % 2 !== 0 && j !== name_array.length - 1) ? temp_name += '\\n': temp_name += ' ';
    }*/
    name_array = name_array[0].split(' ');
    for (var j = 0; j < name_array.length; j++) {
        temp_name += toTitleCase(name_array[j]);
        if (j < name_array[0].length - 1) temp_name += '\\n';
    }
...
TestObject.prototype.getSummary = function() {
    var summary = '';
    if (this.passed_tests == 1) { summary += '1 Success, ' } else { summary += this.passed_tests + ' Successes, ' }
    if (this.failed_tests == 1) { summary += '1 Failure, ' } else { summary += this.failed_tests + ' Failures, ' }
    if (this.skipped_tests == 1) { summary += '1 Skipped' } else { summary += this.skipped_tests + ' Skipped' }

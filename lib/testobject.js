...
TestObject.prototype.generateTable = function (suiteNumber) {
    ...
    tempTestDetailsTable += '<th class="col-lg-9">Details</th></tr>';
    for(var j in this.testcases) {
        if(this.testcases[j].result == 'Failed') {
            tempTestDetailsTable += '<tr class="danger"><td>'+this.testcases[j].name +'</td>';
            tempTestDetailsTable += '<td id="td-table-test-fail" class="text-center tdFix">Failed</td><td class="less tdFix">' +
                '<div class="cellContents">' + this.testcases[j].details + '</div></td></tr>';
        }
        else if(this.testcases[j].result == 'Skipped') {
            tempTestDetailsTable += '<tr class="warning"><td>'+this.testcases[j].name +'</td>';
            tempTestDetailsTable += '<td class="text-center">Skipped</td><td class="less tdFix"><div class="cellContents">' + this.testcases[j].details + '</div></td></tr>';
        }
        else if(this.testcases[j].result == 'Error') {
            tempTestDetailsTable += '<tr class="warning"><td>'+this.testcases[j].name +'</td>';
            tempTestDetailsTable += '<td class="text-center">Error</td><td class="less tdFix"><div class="cellContents">' + this.testcases[j].details + '</div></td></tr>';
        }
        else{
            tempTestDetailsTable += '<tr class="succeess"><td>'+this.testcases[j].name+'</td>';
            tempTestDetailsTable += '<td id="td-table-test-pass" class="text-center">Passed</td><td></td></tr>'
        }
    }

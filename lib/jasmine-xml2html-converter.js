...
var generateTDTable = function (reportXml) {
  ...
  for (var i = 0; i < totalSuites; i++) {
    var suiteName = testSuites[i].attr.name;
        ...
        var suiteTestFailures = 0;//parseInt(testSuites[i].attr.failures) > 0 ? 1 : 0;
    ...
    // Capture tescase execution details for each testsuite
		for(var j in testcases) {
		  testDetailsTable += '<tr><td>' + testcases[j].attr.name; + '</td>'
			var testFailed = '<ul>';// = testcases[j].childNamed('failure');
            var c = 0;
            testcases[j].eachChild(function(child, index, array){
                testFailed += '<li>';
                var s = child.attr.message;
                //s = s.substring(s.indexOf('[')+1, s.indexOf(']'));
                s = s.replace('Expected ValidationError: ', '');
                var t = s.split(',');
                s = '';
